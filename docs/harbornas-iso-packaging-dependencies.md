...
- Harbor Assistant-only 入口已经收敛完成：removed legacy HarborDesk / removed legacy HarborBot / removed legacy HarborCam 公开入口和 removed legacy `/api/harbordesk/**` 同源前缀；active UI 只保留 `/ui/harbor-assistant`，active WebUI proxy 只保留 `/api/harbor-assistant/**`。
...
- `.82` camera proof: after installing bundled media tools, TP-IPC validation
  passed and `/api/harbor-assistant/cameras/<device-id>/snapshot.jpg` returned
  `200 image/jpeg`.
...
- HarborNAS WebUI 只通过 `/api/harbor-assistant/**` proxy 调 HarborBeacon Admin API。
...
proxy 实现，但必须保持 `/api/harbor-assistant/**` 与 HarborOS 原生 `/api/**`
...
```text
GET    /api/harbor-assistant/state
GET    /api/harbor-assistant/gateway/status
GET    /api/harbor-assistant/admin/notification-targets
POST   /api/harbor-assistant/admin/notification-targets/default
DELETE /api/harbor-assistant/admin/notification-targets/:id
GET    /api/harbor-assistant/hardware/readiness
GET    /api/harbor-assistant/rag/readiness
GET    /api/harbor-assistant/knowledge/settings
PUT    /api/harbor-assistant/knowledge/settings
POST   /api/harbor-assistant/knowledge/index/run
GET    /api/harbor-assistant/knowledge/index/status
GET    /api/harbor-assistant/files/browse
GET    /api/harbor-assistant/harboros/status
GET    /api/harbor-assistant/harboros/im-capability-map
GET    /api/harbor-assistant/models/endpoints
POST   /api/harbor-assistant/models/endpoints
PATCH  /api/harbor-assistant/models/endpoints/:id
POST   /api/harbor-assistant/models/endpoints/:id/test
GET    /api/harbor-assistant/models/policies
PUT    /api/harbor-assistant/models/policies
GET    /api/harbor-assistant/models/local-catalog
GET    /api/harbor-assistant/models/local-downloads
POST   /api/harbor-assistant/models/local-downloads
POST   /api/harbor-assistant/models/local-downloads/:job_id/cancel
POST   /api/harbor-assistant/discovery/scan
POST   /api/harbor-assistant/devices/manual
POST   /api/harbor-assistant/devices/default-camera
PATCH  /api/harbor-assistant/devices/:id
POST   /api/harbor-assistant/defaults
POST   /api/harbor-assistant/devices/:id/credentials
POST   /api/harbor-assistant/devices/:id/rtsp-check
GET    /api/harbor-assistant/devices/:id/evidence
POST   /api/harbor-assistant/devices/:id/validation/run
POST   /api/harbor-assistant/cameras/:id/share-link
POST   /api/harbor-assistant/share-links/:id/revoke
POST   /api/harbor-assistant/cameras/:id/snapshot
GET    /api/harbor-assistant/share-links
```
...
```text
POST /api/harbor-assistant/knowledge/search
...
确认页面请求 /api/harbor-assistant/state
...
确认页面请求 /api/harbor-assistant/knowledge/search
...
GET /api/harbor-assistant/cameras/<device-id>/snapshot.jpg -> 200 image/jpeg
```
...
- HarborBeacon 不直接投递 IM 消息。
- Harbor Assistant `/ui/harbor-assistant` 通过 `/api/harbor-assistant/**` 访问 HarborBeacon Admin API。
- Harbor Assistant search tab 通过真实 `/api/harbor-assistant/knowledge/search` 获取索引结果。
- HarborNAS WebUI `/api/harbor-assistant/**` proxy 不覆盖 HarborOS 原生 `/api/**`。
...
- `POST /api/harbor-assistant/knowledge/search` query `春天的照片` -> one VLM
  content-indexed image, `filename_match_used=false`
- `GET /api/harbor-assistant/knowledge/preview` for that image -> `image/jpeg`
...
- production `/api/harbor-assistant/**` proxy 由 nginx、middleware proxy，还是 HarborNAS WebUI 服务层实现。
