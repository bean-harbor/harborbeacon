...
release_label="${RELEASE_VERSION:-harbornavi-p0-${date_stamp}+riscv64}"
debian_version="${DEBIAN_VERSION:-0.1.0+harbornavi.p0.${date_stamp}.riscv64}"
...
cargo build --release --target "$target" --bin harbornavi-k3-guard-smoke
...
cp "target/${target}/release/harbornavi-k3-guard-smoke" "$pkg_dir/usr/bin/harbornavi-k3-guard-smoke"
chmod 0755 "$pkg_dir/usr/bin/harboros-beacon" "$pkg_dir/usr/bin/harbornavi-k3-guard-smoke"
