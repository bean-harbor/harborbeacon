...
- 镜像默认保持 local-first；NSP / `semantic.router` 固定为 CPU local bootstrap；SiliconFlow 只作为 `llm-cloud-siliconflow` OpenAI-compatible fallback preset，范围限定为明确放行的 `retrieval.answer`。
...
future evaluation candidate: Qwen/Qwen3-0.6B
...
- ISO nginx / middleware 需要保留 active `/api/beacon/* -> 127.0.0.1:4174`，并保留 `/api/harbor-beacon/* -> 127.0.0.1:4174` 作为旧 WebUI/旧 ISO 兼容 alias；HarborGate 入口保持 `/api/harbor-gate/* -> 127.0.0.1:8787`，不承担 state path 迁移。
...
- production `/api/beacon/**` 采用 nginx/service-level same-origin proxy，目标为 HarborBeacon `127.0.0.1:4174`；HarborNAS WebUI 不再新增平行 Harbor Assistant API proxy。
