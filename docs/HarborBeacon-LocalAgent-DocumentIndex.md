...
7. [docs/harbor-assistant-offline-delivery-runbook.md](docs/harbor-assistant-offline-delivery-runbook.md)
: HarborAssistant `harborassistant-live-solidify-20260529` 离线交付、安装顺序、回滚和 `.82` 网络阻塞记录。
...
- 模型架构: Runtime Manager + Model Center 是共享能力层，默认走 Harbor-managed Candle-first local runtime；NSP / `semantic.router` 固定本地 CPU bootstrap，cloud fallback 只进入明确放行的 `retrieval.answer`。

## 5.1 当前模型架构权威入口

1. HarborBeacon-Harbor-Collaboration-Contract-v2.md
: 冻结“模型是共享能力层，不是业务域本身”，以及 NSP strict-local 与 cloud fallback 的边界。
2. HarborBeacon-LocalAgent-Plan.md
: 记录 Harbor-managed Candle-first、ISO bootstrap LLM、NSP CPU-only binding、SiliconFlow fallback 与 HF mirror 的当前实现决策。
